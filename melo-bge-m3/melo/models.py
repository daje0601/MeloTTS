...
class TextEncoder(nn.Module):
    def __init__(
        ...
        # Language embedding is optional for BGE-M3 (can be used for explicit control)
        self.use_language_emb = num_languages is not None
        if self.use_language_emb:
            self.language_emb = nn.Embedding(num_languages, hidden_channels)
            nn.init.normal_(self.language_emb.weight, 0.0, hidden_channels**-0.5)

        # Single unified BERT projection for BGE-M3 (1024 dimensions)
    ...
    def forward(self, x, x_lengths, tone, language, bert, g=None):
        # BGE-M3 unified BERT embedding (single BERT for all languages)
        bert_emb = self.bert_proj(bert).transpose(1, 2)

        # Build embeddings
        x = self.emb(x) + self.tone_emb(tone) + bert_emb

        # Add language embedding if enabled (optional for explicit control)
        if self.use_language_emb and language is not None:
            x = x + self.language_emb(language)
...
class SynthesizerTrn(nn.Module):
    ...
    def forward(self, x, x_lengths, y, y_lengths, sid, tone, language, bert):
        ...
        x, m_p, logs_p, x_mask = self.enc_p(
            x, x_lengths, tone, language, bert, g=g_p
        )
