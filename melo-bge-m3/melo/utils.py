...
def get_text_for_tts_infer(text, language_str, hps, device, symbol_to_id=None):
    ...
    if getattr(hps.data, "disable_bert", False):
        # BGE-M3 unified: 1024 dimensions for all languages
        bert = torch.zeros(1024, len(phone))
    else:
        # Get unified BERT features from BGE-M3
        bert = get_bert(norm_text, word2ph, language_str, device)
        del word2ph
        assert bert.shape[-1] == len(phone), phone

        # Ensure correct dimensions (BGE-M3 should always be 1024)
        if bert.shape[0] != 1024:
            print(f"Warning: Expected 1024 dimensions, got {bert.shape[0]}")
            if bert.shape[0] < 1024:
