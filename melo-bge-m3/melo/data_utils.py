...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def get_text(self, text, word2ph, phone, tone, language_str, wav_path):
        ...
        if self.disable_bert:
            # BGE-M3 unified: all languages use 1024 dimensions
            bert = torch.zeros(1024, len(phone))
            ja_bert = torch.zeros(1024, len(phone))  # Kept for compatibility, same as bert
        else:
            # With BGE-M3, all languages use the same dimensions
            # No need to distinguish between languages
            ja_bert = bert  # Both use the same unified BERT features
            # Ensure correct dimensions
            if bert.shape[0] != 1024:
                # Resize if needed (shouldn't happen with BGE-M3)
                bert = torch.nn.functional.pad(bert, (0, 0, 0, 1024 - bert.shape[0]))
        assert bert.shape[-1] == len(phone)
...
class TextAudioSpeakerCollate:
    ...
    def __call__(self, batch):
        ...
        # ja_bert removed - BGE-M3 uses single unified BERT
        ...
        wav_padded.zero_()
        bert_padded.zero_()
        # ja_bert initialization removed
        for i in range(len(ids_sorted_decreasing)):
            ...
            # ja_bert removed in BGE-M3 unified architecture
